...
def transform_features(
    ...
    if 'churn_binary' in df_transformed.columns:
        ...
        churn_mapping: dict[int, str] = {0: 'No Churn', 1: 'Churn'}
    ...
    available_numeric: list[str] = [col for col in numeric_columns if col in df_transformed.columns]
    available_categorical: list[str] = [col for col in categorical_columns if col in df_transformed.columns]
 
    print(f"Available numeric features: {available_numeric}")
    print(f"Available categorical features: {available_categorical}")
 
    # Build preprocessing pipeline
    preprocessor: ColumnTransformer = build_preprocessing_pipeline(available_numeric, available_categorical)
 
    # Fit the preprocessor on the feature data
    features_to_fit: pd.DataFrame = df_transformed[available_numeric + available_categorical]
    ...
    all_features: list[str] = available_numeric + available_categorical
...
def split_features_and_target(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    y: pd.Series[int] = df['target_encoded'].copy()
